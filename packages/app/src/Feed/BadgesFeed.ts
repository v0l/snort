...
import { TaggedRawEvent, EventKind, HexKey, Lists } from "@snort/nostr";
...
export default function useProfileBadges(pubkey?: HexKey) {
  ...
  const q = System.Query<ReplaceableNoteStore>(ReplaceableNoteStore, sub);
  const profileBadges = useNoteStore(q);
  ...
  const q2 = System.Query<FlatNoteStore>(FlatNoteStore, awardsSub);
  const awards = useNoteStore(q2);
