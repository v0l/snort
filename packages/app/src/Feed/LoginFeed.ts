...
export default function useLoginFeed() {
  ...
  const q = System.Query<FlatNoteStore>(FlatNoteStore, subLogin);
  const loginFeed = useNoteStore(q);
  ...
  const qLists = System.Query<FlatNoteStore>(FlatNoteStore, subLists);
  const listsFeed = useNoteStore(qLists);
