...
describe("RequestBuilder", () => {
  describe("basic", () => {
    test("empty filter", () => {
      ...
      b.withFilter();
      expect(b.build()).toEqual(["REQ", "test", {}]);
    });
    test("only kind", () => {
      const b = new RequestBuilder("test");
      b.withFilter().kinds([0]);
      expect(b.build()).toEqual(["REQ", "test", { kinds: [0] }]);
    });
    test("empty authors", () => {
      const b = new RequestBuilder("test");
      b.withFilter().authors([]);
      expect(b.build()).toEqual(["REQ", "test", { authors: [] }]);
    });
    test("authors/kinds/ids", () => {
      ...
      expect(b.build()).toEqual(["REQ", "test", { ids, authors, kinds }]);
    });
    ...
    test("search", () => {
      ...
      expect(b.build()).toEqual(["REQ", "test", { kinds: [1], search: "test-search" }]);
    });
    test("timeline", () => {
      const authors = ["a1", "a2"];
      const kinds = [0, 1, 2, 3];
      ...
      expect(b.build()).toEqual(["REQ", "test", { kinds, authors, until, since }]);
    });
    test("multi-filter timeline", () => {
      ...
      b.withFilter().kinds(kinds).authors(authors).since(since).until(until);
      b.withFilter().kinds(kinds).authors(authors).since(since).until(until);
      expect(b.build()).toEqual(["REQ", "test", { kinds, authors, until, since }, { kinds, authors, until, since }]);
    });
  });
});
