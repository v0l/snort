export default {
  "+D82kt": "هل أنت متأكد من إعادة النشر: {id}",
  "+vIQlC": "يرجى التأكد من حفظ كلمة المرور لتتمكن من إدارة المعرّف الخاص بك في المستقبل",
  "/4tOwT": "تخطي",
  "/JE/X+": "دعم الحساب",
  "/RD0e2":
    "يستخدم نوستر تقنية التوقيع الرقمي لنشر المنشورات دون أي تلاعب ويمكن ارسالها بأمان إلى العديد من الموصلات لتكرار تخزينها وضمان استمرارية المحتوى الخاص بك.",
  "/d6vEc": "اجعل ملفك الشخصي أسهل في العثور عليه ومشاركته",
  "/n5KSF": "{n} مللي ثانية",
  "0BUTMv": "بحث...",
  "0mch2Y": "الاسم يحتوي على أحرف غير مسموح بها",
  "0yO7wF": "{n} ثانية",
  "1A7TZk": "ما هو سنورت وكيف يعمل؟",
  "1Mo59U": "هل أنت متأكد من حذف هذا المنشور من المنشورات المرجعية؟",
  "1udzha": "المحادثات",
  "2/2yg+": "اضافة",
  "25V4l1": "الخلفية",
  "2IFGap": "تبرع",
  "2k0Cv+": "الاستهجان ({n})",
  "3cc4Ct": "ضوء",
  "3gOsZq": "المترجمين",
  "3xCwbZ": "أو",
  "450Fty": "لا أحد",
  "47FYwb": "الغاء",
  "4IPzdn": "المطورين الأساسيين",
  "4L2vUY": "معرف NIP-05 الجديد الخاص بك هو:",
  "4OB335": "استهجان",
  "4Vmpt4": "Nostr Plebs هي واحدة من أوائل مزودي NIP-05 وتقدم مجموعة من المعرفات بأسعار معقولة",
  "4Z3t5i": "استخدم imgproxy لضغط الصور",
  "4rYCjn": "ملاحظة خاصة",
  "5ykRmX": "أرسل ومضة",
  "6ewQqw": "الإعجابات ({n})",
  "6tUqAb":
    "إنشاء زوج مفاتيح عام / خاص. لا تشارك مفتاحك الخاص مع أي شخص ، فهذا بمثابة كلمة المرور الخاصة بك. بمجرد فقده ، لا يمكن &quot;إعادة تعيينه&quot; أو استرداده. حافظ على سلامتك!",
  "7+Domh": "منشورات",
  "7hp70g": "NIP-05",
  "7xzTiH": "{action} إلى {target}",
  "8/vBbP": "إعادة النشر ({n})",
  "89q5wc": "تأكيد إعادة النشر",
  "8E9muH": "استيراد متابعات تويتر (اختياري)",
  "8QDesP": "أومض {n} ساتوشي",
  "8g2vyB": "الاسم طويل جدا",
  "9+Ddtu": "التالي",
  "9HU8vw": "رد",
  "9SvQep": "يتابع {n}",
  "9WRlF4": "ارسال",
  "9gqH2W": "تسجيل الدخول",
  "9wO4wJ": "فاتورة البرق",
  ADmfQT: "السياق",
  ASRK0S: "تم كتم هذا المستخدم",
  Adk34V: "قم بإعداد ملف التعريف الخاص بك",
  AyGauy: "تسجيل الدخول",
  B4C47Y: "الاسم قصير جدا",
  "B6+XJy": "تم الإيماض",
  BOUMjw: "لم يتم العثور على مستخدمي نوستر لـ {twitterUsername}",
  "BOr9z/": "سنورت هو مشروع مفتوح المصدر أنشأه أشخاص هواة في أوقات فراغهم",
  "BcGMo+": "تحتوي المنشورات على محتوى نصي ، والاستخدام الأكثر شيوعًا لهذه المنشورات هو تخزين رسائل تشابه التغريدات.",
  "C81/uG": "تسجيل خروج",
  CHTbO3: "فشل تحميل البرقية",
  "Cu/K85": "مترجم من {lang}",
  D3idYv: "إعدادات",
  DKnriN: "إرسال الساتوشي",
  DZzCem: "عرض أحدث منشورات {n}",
  "Dt/Zd5": "سيتم عرض الوسائط في المنشورات تلقائيًا للأشخاص المحددين ، وإلا سيتم عرض الرابط فقط",
  E8a4yq: "تابع بعض الحسابات المشهورة",
  EPYwm7:
    "مفتاحك الخاص هو كلمة مرورك. إذا فقدت هذا المفتاح ، فستفقد الوصول إلى حسابك! انسخه واحتفظ به في مكان آمن. لا توجد طريقة لإعادة تعيين مفتاحك الخاص.",
  EWyQH5: "عام",
  "Ebl/B2": "ترجمة إلى {lang}",
  EcglP9: "مفتاح",
  EnCOBJ: "شراء",
  Eqjl5K: "فقط سنورت وشركاؤنا المعتمدون يمنحون معرفات ملونة. ولكن يمكنك استخدام مقدمي خدمة آخرون.",
  "F+B3x1": "لقد دخلنا أيضًا في شراكة مع nostrplebs.com لمنحك المزيد من الخيارات",
  FS3b54: "تم!",
  FfYsOb: "حدث خطأ!",
  FmXUJg: "متابع لك",
  "G/yZLu": "حذف",
  GFOoEE: "ملح",
  GL8aXW: "المنشورات المرجعية ({n})",
  GspYR7: "{n} استهجان",
  "H+vHiz": "مفتاح بصيغة سداسية عشرية ..",
  H0JBH6: "تسجيل خروج",
  "H6/kLh": "الطلب مدفوع!",
  HAlOn1: "اسم",
  HFls6j: "سيكون الاسم متاحًا لاحقًا",
  HOzFdo: "كتم",
  HbefNb: "افتح المحفظة",
  IEwZvs: "هل أنت متأكد من ازالة تثبيت هذا المنشور؟",
  IKKHqV: "يتابع",
  INSqIz: "اسم مستخدم تويتر...",
  "IUZC+0":
    "هذا يعني أنه لا يمكن لأي شخص تعديل المنشورات التي قمت بإنشائها ويمكن للجميع بسهولة التحقق من أن المنشورات التي يقرؤونها تم انشاؤها من حسابك.",
  JCIgkj: "اسم المستخدم",
  JHEHCk: "وميض ({n})",
  JkLHGw: "موقع إلكتروني",
  K3r6DQ: "مسح",
  K7AkdL: "عرض",
  KQvWvD: "تم الحذف",
  KWuDfz: "لقد حفظت مفاتيحي ، استمرار",
  KahimY: "نوع الحدث غير معروف: {kind}",
  LgbKvU: "تعليق",
  LxY9tW: "توليد مفتاح",
  M3Oirc: "قوائم التصحيح",
  MBAYRO: 'عرض خيار "نسخ المعرف" و "النسخ بصيغة JSON" في قائمة الخيارات المنسدلة لكل منشور',
  MI2jkA: "غير متاح:",
  MRp6Ly: "اسم مستخدم تويتر",
  MzRYWH: "شراء {item}",
  N2IrpM: "تأكيد",
  NdOYJJ: "حسنًا ، لا شيء هنا .. ألق نظرة على {newUsersPage} لمتابعة بعض الحسابات الموصى بها!",
  NfNk2V: "مفتاحك الخاص",
  NndBJE: "صفحة المستخدمين الجدد",
  OEW7yJ: "وميض",
  OKhRC6: "مشاركة",
  P61BTu: "نسخ بصيغة JSON",
  P7FD0F: "مطابق لاعدادات جهازك",
  P7nJT9: "الإجمالي اليوم (بالتوقيت العالمي ): {amount} ساتوشي",
  PCSt5T: "التفضيلات",
  Pe0ogR: "الثيم",
  PrsIg7: "سيتم عرض ردود الفعل في كل صفحة ، إذا تم تعطيلها فلن تظهر أي ردود فعل",
  QTdJfH: "إنشاء حساب",
  QxCuTo: "الفن بواسطة {name}",
  RDZVQL: "فحص",
  RahCRH: "منتهي الصلاحية",
  RhDAoS: "هل أنت متأكد أنك تريد حذف {id}",
  RoOyAh: "موصّلات",
  Rs4kCE: "المنشورات المرجعية",
  Sjo1P4: "مخصص",
  TpgeGw: "Hex Salt..",
  UQ3pOC:
    "على نوستر ، كثير من الحسابات يحملون نفس اسم المستخدم. أسم المستخدم يختلف عن عن معرف الحساب. يمكنك الحصول على معرّف فريد في الخطوة التالية.",
  Up5U7K: "حظر",
  VOjC1i: "اختر خدمة التحميل التي تريد رفع المرفقات إليها",
  VlJkSk: "تم كتم {n}",
  VnXp8Z: "صورة الحساب ",
  "VtPV/B": "تسجيل الدخول مع ملحق (NIP-07)",
  Vx7Zm2: "كيف تعمل المفاتيح؟",
  W9355R: "رفع الكتم",
  WONP5O: "ابحث عن متابعيك على تويتر على نوستر (البيانات مقدمة من {provider})",
  WxthCV: "على سبيل المثال جاك",
  XgWvGA: "تفاعل",
  Y31HTH: "ساعد في تمويل تطوير سنورت",
  YDURw6: "رابط الخدمة",
  YXA3AH: "تمكين التفاعل",
  ZKORll: "نشط الآن",
  ZLmyG9: "المساهمون",
  ZUZedV: "تبرع البرق:",
  a5UPxh: "ادعم المطورين والمنصات التي تقدم خدمات التحقق من NIP-05",
  aWpBzj: "عرض المزيد",
  brAXSu: "اختر اسم مستخدم",
  bxv59V: "الآن",
  c35bj2: "إذا كان لديك استفسار حول طلب NIP-05 الخاص بك ، فيرجى مراسلة {link} على الخاص",
  cQfLWb: "URL ..",
  cWx9t8: "كتم الكل",
  cuV2gK: "الاسم مسجل",
  cyR7Kh: "الخلف",
  "d7d0/x": "عنوان البرق",
  dOQCL8: "الاسم",
  e7qqly: "تمت قراءة الكل",
  eHAneD: "رد فعل تعبيري",
  eJj8HD: "توثيق الحساب",
  eR3YIn: "منشورات",
  fWZYP5: "مثبّت",
  filwqD: "يقرأ",
  flnGvv: "ما الذي يدور في ذهنك؟",
  "fsB/4p": "تم الحفظ",
  "g5pX+a": "عن",
  gBdUXk: "احفظ مفاتيحك!",
  gDZkld: "سنورت هو واجهة لنوستر البروتوكول اللامركزي لحفظ وتشر المنشورات.",
  gDzDRs: "يتم إرسال الرموز التعبيرية للتفاعل مع منشور",
  gjBiyj: "تحميل...",
  hCUivF: "سيتم تحميل المنشورات تلقائيا في تبويب عام وتبويب المتشورات",
  hK5ZDk: "العالم",
  hMzcSq: "رسائل",
  hY4lzx: "يدعم",
  hicxcO: "عرض الردود",
  iCqGww: "التفاعل ({n})",
  iDGAbc: "احصل على معرف سنورت",
  iGT1eE: "منع الحسابات المزيفة من انتحال حسابك",
  iNWbVV: "معرف",
  ieGrWo: "متابعة",
  itPgxd: "الملف التعريفي",
  izWS4J: "الغاء المتابعة",
  "jA3OE/": "{n,plural,=1{{n} sat} other{{n} sats}}",
  jCA7Cw: "معاينة على سنورت",
  jfV8Wr: "الخلف",
  juhqvW: "تحسين أمان تسجيل الدخول باستخدام ملحقات المتصفح",
  jvo0vs: "حفظ",
  k2veDA: "اكتب",
  k7sKNy: "تساعد خدمة التوثيق NIP-05 الخاصة بنا في دعم تطوير هذا الموقع والحصول على شارة خاصة لامعة على موقعنا!",
  lCILNz: "اشتر الآن",
  "lD3+8a": "دفع",
  lgg1KN: "صفحة الحساب",
  ll3xBp: "خدمة وكيل الصور",
  lnaT9F: "المتابَعون {n}",
  lvlPhZ: "دفع البرقية",
  mH91FY: "سيحصل كل مساهم على نسبة مئوية من جميع التبرعات وأوامر NIP-05 ، يمكنك رؤية المبالغ المقسمة أدناه",
  mKAr6h: "تابع الكل",
  mKh2HS: "خدمة تحميل الملفات",
  mKhgP9: "{n,plural,=0{} =1{zapped} other{zapped}}",
  n1xHAH: "الحصول على معرف (اختياري)",
  nDejmx: "رفع الحظر",
  nGBrvw: "المنشورات المرجعية",
  nN9XTz: "شارك أفكارك مع {link}",
  nn1qb3: "نقدر لكم سخاء تبرعاتكم",
  nwZXeh: "محظور {n}",
  o6Uy3d: "يمكن استخدام المفتاح السري للنشر (وتوقيع الأحداث) ، كل شيء آخر يعمل في وضع القراءة فقط.",
  "o7e+nJ": "{n} المتابعين",
  odFwjL: "يتبع فقط",
  odhABf: "تسجيل الدخول",
  osUr8O: "يمكنك أيضًا استخدام هذه الإضافات لتسجيل الدخول إلى معظم مواقع Nostr.",
  oxCa4R:
    "يساعد الحصول على معرّف في تأكيد هويتك للأشخاص الذين يعرفونك. يمكن أن يكون لدى العديد من الأشخاص اسم مستخدمjack ، ولكن لا يوجد سوى jack@cash.app.",
  puLNUJ: "تثبيت",
  pzTOmv: "المتابعين",
  qUJTsT: "محظور",
  qdGuQo: "مفتاحك الخاص هو (لا تشارك هذا مع أي شخص)",
  qkvYUb: "أضف إلى الملف الشخصي",
  qmJ8kD: "فشلت الترجمة",
  "r3C4x/": "برنامج",
  rT14Ow: "إضافة موصّلات",
  reJ6SM: "يوصى باستخدام أحد ملحقات المستعرض التالية إذا كنت تستخدم جهاز كمبيوتر لتأمين مفتاحك:",
  rfuMjE: "(افتراضي)",
  rrfdTe: "هذه نفس التقنية التي تستخدم في البتكوين وثبت أنها آمنة للغاية.",
  rudscU: "فشل تحميل ما يلي ، يرجى المحاولة مرة أخرى في وقت لاحق",
  sWnYKw: "تم تصميم سنورت للحصول على تجربة مماثلة لتويتر.",
  svOoEH:
    "غير مسموح بانتحال الأسماء والشخصيات. يحتفظ سنورت وشركاؤنا بالحق في الغاء معرّف التوثيق الخاص بك (وليس حسابك - لا يمكن لأحد أن يفعل ذلك) لانتهاك هذه القاعدة.",
  tOdNiY: "داكن",
  thnRpU: "يمكن أن يساعد الحصول على معرف التوثيق NIP-05 في:",
  "u/vOPu": "مدفوع",
  u4bHcR: "تحقق من الكود هنا: {link}",
  "uD/N6c": "ومّض {target} {n} ساتوشي",
  uSV4Ti: "إعادة النشر تحتاج إلى تأكيد يدويًا",
  usAvMr: "تعديل الملف الشخصي",
  "ut+2Cd": "احصل على معرف الشريك",
  vOKedj: "{n,plural,=1{& {n} other} other{& {n} others}}",
  vZ4quW: "NIP-05 هو أحد طرق التحقق المستندة إلى DNS والتي تساعد في التحقق من هويتك كمستخدم حقيقي.",
  wEQDC6: "تحرير",
  wLtRCF: "مفتاحك",
  wih7iJ: "الاسم محظور",
  "wqyN/i": "اكتشف المزيد من المعلومات حول {service} على {link}",
  wtLjP6: "نسخ المعرف",
  x82IOl: "كتم",
  xIoGG9: "اذهب إلى",
  xJ9n2N: "مفتاحك العام",
  xKdNPm: "ارسال",
  xKflGN: "يتابع {username} على نوستر",
  xbVgIm: "تحميل الوسائط تلقائيًا",
  xmcVZ0: "البحث",
  zFegDD: "تواصل",
  zINlao: "مالك",
  zQvVDJ: "الكل",
  zcaOTs: "ومّض ساتوشي",
  zjJZBd: "انت جاهز!",
  zonsdq: "فشل تحميل خدمة LNURL",
  zvCDao: "تظهر تلقائيا أحدث الملاحظات",
};
